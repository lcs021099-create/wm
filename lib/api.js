...
// 一律走同源（Vercel 內建的 Next.js API 路由）。
// 注意：這裡刻意寫死空字串 = 相對路徑，忽略 NEXT_PUBLIC_API_URL，
// 因為架構已改為前後端同源；舊的 tunnel 環境變數會導致 404。
const API_URL = '';
...
export const palletsAPI = {
  list: (params) => api.get('/api/pallets', { params }),
  stats: () => api.get('/api/pallets/stats'),
  get: (id) => api.get(`/api/pallets/${id}`),
  getByBarcode: (code) => api.get(`/api/pallets/barcode/${encodeURIComponent(code)}`),
  create: (data) => api.post('/api/pallets', data),
  update: (id, data) => api.put(`/api/pallets/${id}`, data),
  out: (id, data) => api.post(`/api/pallets/${id}/out`, data),
  delete: (id) => api.delete(`/api/pallets/${id}`),
  transactions: () => api.get('/api/inventory/transactions'),
};
